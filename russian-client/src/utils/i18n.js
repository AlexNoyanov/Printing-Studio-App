...
// Get current language (saved preference > user account > browser language > default)
// For client version, default is Russian; for other versions, default is English
function getCurrentLanguage() {
  ...
  // Check if we're in client version (Firebase version)
  // If using main.firebase.js, default to Russian
  const isClientVersion = window.location.pathname === '/' || 
                         document.querySelector('script[src*="main.firebase"]') !== null ||
                         localStorage.getItem('clientVersion') === 'true'
  
  if (isClientVersion) {
    // For client version, default to Russian
    const browserLang = getBrowserLanguage()
    currentLanguageState = browserLang === 'ru' ? 'ru' : 'ru' // Always default to Russian for client
    return 'ru'
  }
  
  // Fallback to browser language for other versions
  const browserLang = getBrowserLanguage()
